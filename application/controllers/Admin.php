...
class Admin extends CI_Controller
{
  ...
  //       }
	...
	public function addElpay(){
		$id = $this->input->post('id');
		$nominal_sekarang = $this->Elpay_model->lihatSaldo($id);
		$nominal_tambah = $this->input->post('nominalTambah');
		
		$nominal_akhir = $nominal_sekarang[0]->saldo_elpay + $nominal_tambah;

		
		$hasil = $this->Elpay_model->updateElpay($id, $nominal_akhir);
		
		redirect('admin/transaksi', 'refresh');
	}

	public function substractElpay(){
		$id = $this->input->post('id');
		$nominal_sekarang = $this->Elpay_model->lihatSaldo($id);
		$nominal_kurang = $this->input->post('nominalKurang');
		
		
		$nominal_akhir = $nominal_sekarang - $nominal_kurang;
